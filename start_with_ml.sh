...
wait_for_service() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if curl -4 -s "$url" > /dev/null 2>&1; then
...
if ! wait_for_service "http://127.0.0.1:8000/health" "ML Service"; then
...
cd server
# Start backend directly with node to avoid nodemon background quirks
PORT=5001 NODE_ENV=development CLIENT_URL=http://localhost:5173 ML_API_URL=http://127.0.0.1:8000 MONGODB_URI=mongodb://127.0.0.1:27017/green-points node index.js > ../logs/backend.out 2>&1 &
BACKEND_PID=$!
echo "🔄 Backend started with PID: $BACKEND_PID (logs: logs/backend.out)"
cd ..

# Wait for backend to be ready
if ! wait_for_service "http://127.0.0.1:5001/api/health" "Backend"; then
...
cd client
# Ensure Vite binds to host and fixed port
VITE_API_URL=http://127.0.0.1:5001/api npm run dev -- --host --port 5173 > ../logs/frontend.out 2>&1 &
FRONTEND_PID=$!
echo "🔄 Frontend started with PID: $FRONTEND_PID (logs: logs/frontend.out)"
cd ..

# Wait for frontend to be ready
if ! wait_for_service "http://127.0.0.1:5173" "Frontend"; then
